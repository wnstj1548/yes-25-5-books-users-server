    ...
    public ReadOrderUserInfoResponse orderUserInfo(Long userId) {
        Customer customer = customerRepository.findById(userId)
            .orElseThrow(() -> new CustomerException(
                ErrorStatus.toErrorStatus("고객을 찾을 수 없습니다. 고객 ID : " + userId, 404,
                    LocalDateTime.now())
            ));
        ...
        User user = userRepository.findById(userId)
            .orElseThrow(() -> new UserException(
                ErrorStatus.toErrorStatus("회원이 존재하지 않습니다.", 404, LocalDateTime.now())));

        Point point = pointRepository.findByUser_UserId(userId);

        if (Objects.isNull(point)) {
            throw new PointException(
                ErrorStatus.toErrorStatus("포인트가 존재하지 않습니다.", 404, LocalDateTime.now()));
        }
